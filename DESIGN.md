...
- **FZ Rubik font is committed to a public repo — license exposure to confirm.** FZ Rubik is a proprietary MIDU company font, but the kit repo (`https://github.com/boizdeeptry/MIDU-Guidline-Design`) is currently public with the TTFs committed. The brand/legal owner must either confirm the FZ Rubik license permits public redistribution, or the font must move out of the public tree (make the repo private, or keep fonts as an access-gated release-asset / submodule the build fetches). This is also why the Phase-3 token package will be **private** and **exclude the font**. See THIRD-PARTY-NOTICES.md.
